...
use futarchy_actions::liquidity_init_actions;
...
#[test]
/// Test do_init_create_pool_with_mint via launchpad flow
///
/// This test:
/// 1. Creates a launchpad raise
/// 2. Stages a CreatePoolWithMint action as success spec
/// 3. Contributes to meet minimum
/// 4. Settles and creates DAO (deposits stables into vault)
/// 5. Executes pool creation which mints asset tokens and uses vault stables
fun test_do_init_create_pool_with_mint() {
    let sender = @0xA;
    let contributor = @0xB;
    ...
    // Build pool creation spec
    let mut builder = action_spec_builder::new();
    let asset_amount = 10_000_000_000; // 10k asset tokens to mint
    let stable_amount = 10_000_000_000; // 10k stable from vault (must be <= contribution)
    let fee_bps = 30; // 0.3% fee

    liquidity_init_actions::add_create_pool_with_mint_spec(
        &mut builder,
    ...
    // Contribute enough (must exceed stable_amount for pool)
    contribute(&mut scenario, contributor, CONTRIBUTION_AMOUNT);
    ...
    // Execute the pool creation action
    ts::next_tx(&mut scenario, sender);
    {
        let mut account = ts::take_shared<Account>(&scenario);
        let registry = ts::take_shared<PackageRegistry>(&scenario);
        ...
        // Execute pool creation with mint
        let _pool_id = liquidity_init_actions::do_init_create_pool_with_mint<
            futarchy_core::futarchy_config::FutarchyConfig,
            dao_init_outcome::DaoInitOutcome,
            TEST_ASSET_REGULAR,
            TEST_STABLE_REGULAR,
...
#[test]
/// Test multiple init actions in sequence via launchpad flow
///
/// This exercises the full PTB pattern with multiple actions:
/// 1. CreateStream
/// 2. CreatePoolWithMint
fun test_multiple_init_actions_in_sequence() {
    let sender = @0xA;
    let contributor = @0xB;
    let stream_beneficiary = @0xC;
    ...
    // Build multiple specs: stream + pool
    let mut builder = action_spec_builder::new();

    // Action 1: Create stream (uses 0.5k stable per iteration * 12 iterations = 6k total)
    let stream_amount = 500_000_000; // 0.5k per iteration
    // Start time must be in the future relative to clock time after settlement
    let stream_start_time = constants::launchpad_duration_ms() + 60_000;
    stream_init_actions::add_create_stream_spec(
        &mut builder,
        b"treasury".to_string(),
        stream_beneficiary,
    ...
    // Action 2: Create pool (uses 10k stable + mints 10k asset)
    liquidity_init_actions::add_create_pool_with_mint_spec(
        &mut builder,
        b"treasury".to_string(),
        10_000_000_000, // asset to mint
        10_000_000_000, // stable from vault
        30, // fee_bps
    );
    ...
    // Contribute enough for stream + pool
    // Stream: 0.5k * 12 = 6k reserved
    // Pool: 10k stable
    // Total needed: ~17k, we contribute 30k
    contribute(&mut scenario, contributor, CONTRIBUTION_AMOUNT);
    ...
    ts::next_tx(&mut scenario, sender);
    {
        ...
        // Action 2: Create pool with mint
        let _pool_id = liquidity_init_actions::do_init_create_pool_with_mint<
            futarchy_core::futarchy_config::FutarchyConfig,
            dao_init_outcome::DaoInitOutcome,
            TEST_ASSET_REGULAR,
